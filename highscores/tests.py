...
from highscores.views import get_monthly_highscore
import datetime
...
class BrewerTestCase(TestCase):
	def setUp(self):
		CoffeeBrewer.objects.create(datetime = timezone.now(), RFID = '1111', name = 'Anders')
		CoffeeBrewer.objects.create(datetime = timezone.now(), RFID = '1112', name = 'Alexander')
		CoffeeBrewer.objects.create(datetime = timezone.now(), RFID = '1113', name = 'Magnus')
	...
	def test_monthly_highscores(self):
		self.assertEqual(get_monthly_highscore(), [(1112, 3), (1111, 2), (1113, 1)])
